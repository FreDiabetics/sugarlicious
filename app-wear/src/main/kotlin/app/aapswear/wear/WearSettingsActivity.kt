...
class WearSettingsActivity : Activity() {
    ...
    private fun buildUi() {
        ...
        settingsCategory("display", "Anzeige", "App-Flächen, Texte und Therapiefarben") {
            ...
            colorRow(AppearanceTerminology.SURFACE_BACKGROUND, current.uiColors.tileBackground) {
                updateUiColors { c ->
                    c.copy(tileBackground = it)
                }
            }
            ...
            colorRow(AppearanceTerminology.SECONDARY_TEXT, current.uiColors.textSecondary) {
                updateUiColors { c ->
                    c.copy(textSecondary = it)
                }
            }
        ...
        settingsCategory("glucose", "Glukose", "Einheit und Bereichsfarben") {
            ...
            colorRow(AppearanceTerminology.GLUCOSE_IN_RANGE, current.uiColors.glucoseInRange) {
                updateUiColors { c ->
                    c.copy(glucoseInRange = it)
                }
            }
            colorRow(AppearanceTerminology.GLUCOSE_HIGH, current.uiColors.glucoseHigh) { updateUiColors { c -> c.copy(glucoseHigh = it) } }
            section("GRÖSSE")
            root.addView(
                sliderCard("Glukosewert", GlucoseTrendSizing.MIN_SCALE_PERCENT, GlucoseTrendSizing.MAX_SCALE_PERCENT, current.glucoseScalePercent, {
                    "$it %"
                }) {
            ...
            root.addView(
                sliderCard("Trendpfeil", GlucoseTrendSizing.MIN_SCALE_PERCENT, GlucoseTrendSizing.MAX_SCALE_PERCENT, current.trendScalePercent, {
                    "$it %"
                }) {
                    save(
                        current.copy(trendScalePercent = it, trendArrowStyle = current.trendArrowStyle.copy(sizePercent = it)),
                        rebuild = false,
                    )
                },
                cardParams(),
            ...
            root.addView(
                switchRow("Trendpfeil-Kontur", current.trendArrowStyle.outlineEnabled) {
            ...
            if (current.trendArrowStyle.outlineEnabled) {
                ...
                root.addView(
                    sliderCard(
                        ...
                        25,
                        400,
                        (current.trendArrowStyle.outlineThicknessDp * 100).roundToInt(),
                        { "${it / 100f} dp" },
                    ) {
            ...
            root.addView(
                sliderCard("Trend-Deckkraft", 0, 100, (current.trendArrowStyle.alpha * 100).roundToInt(), { "$it %" }) {
                    save(current.copy(trendArrowStyle = current.trendArrowStyle.copy(alpha = it / 100f)), rebuild = false)
                },
                cardParams(),
            )
            root.addView(
                actionCard("Trend-Stil zurücksetzen", "Light/Dark getrennt") {
        ...
        settingsCategory("graph", "Graph", "Zeitraum, Punkte, Linien und Prognosen") {
            ...
            root.addView(
                switchRow("Prognosen anzeigen", current.showPredictions) { save(current.copy(showPredictions = it)) },
                cardParams(),
            )
            root.addView(
            ...
            root.addView(
                sliderCard(
                    ...
                    save(
                        current.copy(graphStyle = current.graphStyle.copy(cgmDotRadiusDp = progress / 10f)),
                        rebuild = false,
                    )
                },
                cardParams(),
            ...
            root.addView(
                switchRow("Kontur · bisherige Punkte", current.graphStyle.cgmHistoricalDotOutlineEnabled) {
                    save(
                        current.copy(
                            graphStyle = current.graphStyle.copy(cgmDotOutlineEnabled = true, cgmHistoricalDotOutlineEnabled = it),
                        ),
                    )
                },
                cardParams(),
            ...
            root.addView(
                switchRow("Kontur · aktueller Wert", current.graphStyle.cgmCurrentDotOutlineEnabled) {
            ...
            root.addView(
                sliderCard(
                    ...
                    save(
                        current.copy(graphStyle = current.graphStyle.copy(cgmDotOutlineWidthDp = progress / 100f)),
                        rebuild = false,
                    )
                },
                cardParams(),
            ...
            root.addView(
                sliderCard(
                    ...
                    save(
                        current.copy(graphStyle = current.graphStyle.copy(scaleLaneOpacityPercent = progress)),
                        rebuild = false,
                    )
                },
                cardParams(),
            ...
            section("FARBEN")
            colorRow(AppearanceTerminology.GRAPH_BACKGROUND, current.graphColors.graphBackground) {
                updateGraphColors { c ->
                    c.copy(graphBackground = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_LOW_AREA, current.graphColors.rangeLow) {
                updateGraphColors { c ->
                    c.copy(rangeLow = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_TARGET_AREA, current.graphColors.rangeInRange) {
                updateGraphColors { c ->
                    c.copy(rangeInRange = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_HIGH_AREA, current.graphColors.rangeHigh) {
                updateGraphColors { c ->
                    c.copy(rangeHigh = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_DOT_LOW, current.graphColors.cgmLow) { updateGraphColors { c -> c.copy(cgmLow = it) } }
            colorRow(AppearanceTerminology.GRAPH_DOT_IN_RANGE, current.graphColors.cgmInRange) {
                updateGraphColors { c ->
                    c.copy(cgmInRange = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_DOT_HIGH, current.graphColors.cgmHigh) { updateGraphColors { c -> c.copy(cgmHigh = it) } }
            colorRow(AppearanceTerminology.GRAPH_HIGH_LINE, current.graphColors.highLine) {
                updateGraphColors { c ->
                    c.copy(highLine = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_LOW_LINE, current.graphColors.lowLine) { updateGraphColors { c -> c.copy(lowLine = it) } }
            colorRow(AppearanceTerminology.GRAPH_AXIS_TEXT, current.graphColors.axisLabel) {
                updateGraphColors { c ->
                    c.copy(axisLabel = it)
                }
            }
            colorRow(AppearanceTerminology.GRAPH_AXIS_TICK, current.graphColors.axisTick) {
                updateGraphColors { c ->
                    c.copy(axisTick = it)
                }
            }
            ...
            section("PROGNOSEN")
            colorRow(AppearanceTerminology.PREDICTION_IOB, current.graphColors.predictionIob) {
                updateGraphColors { c ->
                    c.copy(predictionIob = it)
                }
            }
            colorRow(AppearanceTerminology.PREDICTION_COB, current.graphColors.predictionCob) {
                updateGraphColors { c ->
                    c.copy(predictionCob = it)
                }
            }
            colorRow(AppearanceTerminology.PREDICTION_UAM, current.graphColors.predictionUam) {
                updateGraphColors { c ->
                    c.copy(predictionUam = it)
                }
            }
            colorRow(AppearanceTerminology.PREDICTION_ZERO_TEMP, current.graphColors.predictionZeroTemp) {
                updateGraphColors { c ->
                    c.copy(predictionZeroTemp = it)
                }
            }
        }
        ...
        settingsCategory("connection", "Verbindung", "Synchronisierung und Hintergrundzugriff") {
            root.addView(
                actionCard("Akku-Optimierung", if (WearBackgroundAccess.isBatteryUnrestricted(this)) "Freigegeben" else "Prüfen") {
        ...
        settingsCategory("diagnostics", "Diagnose", "Status, Freshness und Datenquelle") {
            root.addView(
                infoCard("Live-Status", "Zurück in der Übersicht werden Quelle, Alter und Verbindungsstatus angezeigt."),
                cardParams(),
            )
        }
    ...
    private fun updateTileColors(
        ...
        WearTileAppearanceStore.write(
            ...
            kind,
            selectedAppearanceMode,
            transform(WearTileAppearanceStore.read(this, kind, selectedAppearanceMode)),
        )
    ...
    private fun showColorPicker(
        ...
        SharedColorEditor.show(
            ...
            title,
            selected,
            current.uiColors.tileBackground,
            current.uiColors.textPrimary,
            current.uiColors.tileBorder,
            selected,
            onChange = changed,
    ...
    private fun formatGlucoseThreshold(
        valueMgDl: Int,
        unit: WatchGlucoseUnit,
    ): String =
    ...
    private fun compactActionBackground(): GradientDrawable =
    ...
    private fun cardBackground(radiusDp: Float = 20f): GradientDrawable =
        GradientDrawable().apply {
    ...
    private fun pillBackground(selected: Boolean): GradientDrawable =
        GradientDrawable().apply {
            ...
            setColor(
                if (selected) {
                    this@WearSettingsActivity.current.uiColors.accent
                } else {
