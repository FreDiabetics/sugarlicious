...
class G7AppearanceActivity : Activity() {
    ...
    private fun render() {
        ...
        content.addView(
            label("GRÖSSE", 10f, palette.argb(G7AppearanceRole.MENU_PRIMARY), true).apply {
                letterSpacing = 0.10f
        ...
        content.addView(
            simpleColorRow("Trendpfeil · Füllfarbe", trendStyle.fillColor, palette) {
                SharedColorEditor.show(
                    this,
                    "Trendpfeil · Füllfarbe",
                    trendStyle.fillColor,
        ...
        content.addView(
            toggleRow("Trendpfeil-Kontur", trendStyle.outlineEnabled, palette) {
        ...
        if (trendStyle.outlineEnabled) {
            content.addView(
                simpleColorRow("Trendpfeil · Konturfarbe", trendStyle.outlineColor, palette) {
                    SharedColorEditor.show(
                        this,
                        "Trendpfeil · Konturfarbe",
                        trendStyle.outlineColor,
                        palette.argb(G7AppearanceRole.MENU_SURFACE),
                        palette.argb(G7AppearanceRole.MENU_TEXT_PRIMARY),
                        palette.argb(G7AppearanceRole.MENU_BORDER),
                        ...
                        onReset = {
                            store.saveTrendArrowStyle(
                                selectedMode,
                                trendStyle.copy(outlineColor = TrendArrowStyle.defaults(selectedMode, trendStyle.fillColor).outlineColor),
                            )
                            render()
                        },
                    )
            ...
            content.addView(
                scaleRow("Konturdicke", (trendStyle.outlineThicknessDp * 100).roundToInt(), palette, {
        ...
        content.addView(
            scaleRow("Deckkraft", (trendStyle.alpha * 100).roundToInt(), palette, {
                store.saveTrendArrowStyle(selectedMode, trendStyle.copy(alpha = it / 100f))
            }, min = 0, max = 100, format = { "Deckkraft · $it %" }),
            params(top = 5),
        )
        content.addView(
            pill("TREND-STIL RESET", palette.argb(G7AppearanceRole.MENU_SURFACE), palette.argb(G7AppearanceRole.MENU_PRIMARY)) {
                store.resetTrendArrowStyle(selectedMode)
                render()
            },
            params(top = 5),
        ...
        content.addView(
            toggleRow("Kontur · bisherige Punkte", store.historicalDotOutlineEnabled(), palette) {
                store.setHistoricalDotOutlineEnabled(it)
            },
            params(top = 5),
        )
        content.addView(
            toggleRow("Kontur · aktueller Wert", store.currentDotOutlineEnabled(), palette) {
        ...
        content.addView(
            pill("ALLES RESET", palette.argb(G7AppearanceRole.MENU_PRIMARY), palette.argb(G7AppearanceRole.MENU_BACKGROUND)) {
                store.resetAll()
                render()
            },
            params(top = 14),
    ...
    private fun colorRow(
        ...
        LinearLayout(this).apply {
            ...
            val swatch =
                View(this@G7AppearanceActivity).apply {
            ...
            addView(
                LinearLayout(this@G7AppearanceActivity).apply {
                    orientation = LinearLayout.VERTICAL
                    addView(
                        label(role.label, 12f, palette.argb(G7AppearanceRole.MENU_TEXT_PRIMARY), true).apply { gravity = Gravity.START },
                    )
                    addView(
                        label(toHex(palette.argb(role)), 9f, palette.argb(G7AppearanceRole.MENU_TEXT_SECONDARY)).apply {
                            gravity =
                                Gravity.START
                        },
                    )
            ...
            addView(
                pill("RESET", palette.argb(G7AppearanceRole.MENU_SURFACE), palette.argb(G7AppearanceRole.MENU_PRIMARY)) {
    ...
    private fun scaleRow(
        ...
        addView(valueLabel)
        addView(
            SeekBar(this@G7AppearanceActivity).apply {
                this.max = max - min
                progress = initial.coerceIn(min, max) - min
                progressTintList =
                    android.content.res.ColorStateList
                        .valueOf(palette.argb(G7AppearanceRole.MENU_PRIMARY))
                progressBackgroundTintList =
                    android.content.res.ColorStateList
                        .valueOf(palette.argb(G7AppearanceRole.MENU_BORDER))
                thumbTintList =
                    android.content.res.ColorStateList
                        .valueOf(palette.argb(G7AppearanceRole.MENU_PRIMARY))
    ...
    private fun toggleRow(
        ...
        background = rounded(palette.argb(G7AppearanceRole.MENU_SURFACE), palette.argb(G7AppearanceRole.MENU_BORDER), 18f)
        addView(
            label(title, 11f, palette.argb(G7AppearanceRole.MENU_TEXT_PRIMARY), true),
            LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f),
        )
        addView(
            android.widget.Switch(this@G7AppearanceActivity).apply {
                isChecked = checked
    ...
    private fun simpleColorRow(
        ...
        addView(
            View(this@G7AppearanceActivity).apply {
                background = rounded(color, palette.argb(G7AppearanceRole.MENU_BORDER), 12f)
    ...
    private fun themeSelector(palette: G7AppearancePalette) =
        LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
