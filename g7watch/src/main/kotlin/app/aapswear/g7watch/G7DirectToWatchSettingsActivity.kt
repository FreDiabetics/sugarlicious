...
class G7DirectToWatchSettingsActivity : Activity() {
    ...
    private fun render() {
        ...
        root.addView(
            toggle("Zuckerwert fett", settings.glucoseBold(), palette) {
                settings.saveGlucoseBold(it)
            },
            params(5),
        )
        root.addView(
            colorRow(AppearanceTerminology.DELTA_UNIT, settings.deltaUnitColor(mode), palette) {
        ...
        root.addView(
            slider("Größe", 75, 150, settings.statusSizePercent(), palette, { "Größe · $it %" }) {
        ...
        root.addView(
            slider("Größe", 75, 150, settings.clockSizePercent(), palette, { "Größe · $it %" }) {
        ...
        root.addView(
            slider("Punktgröße", 15, 60, (graphStyle.dotRadiusDp * 10).roundToInt(), palette, { "Punktgröße · ${it / 10f} dp" }) {
                settings.saveGraphStyle(settings.graphStyle().copy(dotRadiusDp = it / 10f))
            },
            params(5),
        )
        root.addView(
            toggle("Kontur · bisherige Punkte", graphStyle.historicalDotOutlineEnabled, palette) {
                settings.saveGraphStyle(settings.graphStyle().copy(historicalDotOutlineEnabled = it))
            },
            params(5),
        )
        root.addView(
            toggle("Kontur · aktueller Wert", graphStyle.currentDotOutlineEnabled, palette) {
        ...
        root.addView(
            slider(
                ...
                25,
                300,
                (graphStyle.dotOutlineWidthDp * 100).roundToInt(),
                palette,
                { "Konturbreite · ${it / 100f} dp" },
            ) {
        ...
        root.addView(
            slider("Eckenrundung", 0, 400, (graphStyle.cornerRadiusDp * 10).roundToInt(), palette, { "Eckenrundung · ${it / 10f} dp" }) {
                settings.saveGraphStyle(settings.graphStyle().copy(cornerRadiusDp = it / 10f))
            },
            params(5),
        )
        root.addView(
            toggle("Graphkontur", graphStyle.borderEnabled, palette) {
                settings.saveGraphStyle(settings.graphStyle().copy(borderEnabled = it))
            },
            params(5),
        )
        root.addView(
            toggle("Zeitachsenskala", graphStyle.timeAxisEnabled, palette) {
                settings.saveGraphStyle(settings.graphStyle().copy(timeAxisEnabled = it))
            },
            params(5),
        )
        root.addView(
            slider("Skalenbereich", 0, 100, graphStyle.scaleLaneOpacityPercent, palette, { "Skalenbereich · $it %" }) {
        ...
        root.addView(
            button("GRAPH ZURÜCKSETZEN", palette) {
                settings.resetGraph()
                render()
            },
            params(8),
        ...
        root.addView(
            slider("Größe", GlucoseTrendSizing.MIN_SCALE_PERCENT, GlucoseTrendSizing.MAX_SCALE_PERCENT, trend.sizePercent, palette, {
                "Größe · $it %"
            }) {
        ...
        root.addView(
            colorRow(
                ...
                trend.fillColor,
                palette,
            ) { value -> settings.saveTrendStyle(mode, settings.trendStyle(mode).copy(fillColor = value)) },
            params(5),
        )
        root.addView(
            toggle("Kontur", trend.outlineEnabled, palette) {
                settings.saveTrendStyle(mode, settings.trendStyle(mode).copy(outlineEnabled = it))
            },
            params(5),
        )
        root.addView(
            colorRow(
                ...
                trend.outlineColor,
                palette,
            ) { value -> settings.saveTrendStyle(mode, settings.trendStyle(mode).copy(outlineColor = value)) },
            params(5),
        )
        root.addView(
            slider("Konturdicke", 25, 400, (trend.outlineThicknessDp * 100).roundToInt(), palette, { "Konturdicke · ${it / 100f} dp" }) {
                settings.saveTrendStyle(mode, settings.trendStyle(mode).copy(outlineThicknessDp = it / 100f))
            },
            params(5),
        )
        root.addView(
            slider("Deckkraft", 0, 100, (trend.alpha * 100).roundToInt(), palette, { "Deckkraft · $it %" }) {
                settings.saveTrendStyle(mode, settings.trendStyle(mode).copy(alpha = it / 100f))
            },
            params(5),
        )
        root.addView(
            button("TREND-STIL ZURÜCKSETZEN", palette) {
                settings.resetTrendStyle(mode)
                render()
            },
            params(8),
        )
    ...
    private fun graphColorRows(
        root: LinearLayout,
        c: WatchGraphColors,
        p: G7AppearancePalette,
    ) {
        fun add(
            title: String,
            value: Int,
            update: (WatchGraphColors, Int) -> WatchGraphColors,
    ...
    private fun restyleChoices(
        row: LinearLayout,
        selected: Any,
        p: G7AppearancePalette,
    ) {
        for (index in 0 until row.childCount) {
            val choice = row.getChildAt(index) as? TextView ?: continue
            ...
            choice.background =
                rounded(
                    if (active) p.argb(G7AppearanceRole.MENU_PRIMARY) else p.argb(G7AppearanceRole.MENU_SURFACE),
                    p.argb(G7AppearanceRole.MENU_BORDER),
                    999f,
                )
        }
    }
    ...
    private fun numericThresholdRow(
        title: String,
        valueMgDl: Double,
        isHigh: Boolean,
        p: G7AppearancePalette,
    ...
    private fun showTargetThresholdEditor(
        title: String,
        currentMgDl: Double,
        isHigh: Boolean,
        p: G7AppearancePalette,
        ...
        val shown =
            if (unit ==
                ...
                String.format(java.util.Locale.US, "%.1f", currentMgDl / 18.0)
            } else {
                currentMgDl.roundToInt().toString()
            }
        val input =
            EditText(this).apply {
        ...
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                ...
                val updated =
                    mgDl
                        ?.let { if (isHigh) current.copy(highMgDl = it) else current.copy(lowMgDl = it) }
    ...
    private fun slider(
        ...
        background = card(p)
        val value = label(format(initial), 11f, p.argb(G7AppearanceRole.MENU_TEXT_PRIMARY), true)
        addView(value)
        ...
        addView(
            SeekBar(this@G7DirectToWatchSettingsActivity).apply {
                this.max = max - min
                progress = initial.coerceIn(min, max) - min
                progressTintList =
                    android.content.res.ColorStateList
                        .valueOf(p.argb(G7AppearanceRole.MENU_PRIMARY))
                progressBackgroundTintList =
                    android.content.res.ColorStateList
                        .valueOf(p.argb(G7AppearanceRole.MENU_BORDER))
                thumbTintList =
                    android.content.res.ColorStateList
                        .valueOf(p.argb(G7AppearanceRole.MENU_PRIMARY))
    ...
    private fun toggle(
        ...
        background = card(p)
        addView(
            label(title, 11f, p.argb(G7AppearanceRole.MENU_TEXT_PRIMARY), true),
            LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f),
        )
        addView(
            Switch(this@G7DirectToWatchSettingsActivity).apply {
                isChecked = checked
    ...
    private fun colorRow(
        ...
        val swatch =
            View(this@G7DirectToWatchSettingsActivity).apply {
                background =
                    rounded(color, p.argb(G7AppearanceRole.MENU_BORDER), 10f)
            }
        addView(
            LinearLayout(this@G7DirectToWatchSettingsActivity).apply {
                orientation = LinearLayout.VERTICAL
                addView(label(title, 11f, p.argb(G7AppearanceRole.MENU_TEXT_PRIMARY), true))
                addView(valueLabel)
            },
            LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f),
        )
        ...
        setOnClickListener {
            SharedColorEditor.show(
                this@G7DirectToWatchSettingsActivity,
                title,
                color,
                p.argb(
                    G7AppearanceRole.MENU_SURFACE,
                ),
                p.argb(G7AppearanceRole.MENU_TEXT_PRIMARY),
                p.argb(G7AppearanceRole.MENU_BORDER),
                color,
                onChange = { changed ->
                    save(changed)
    ...
    private fun section(
        ...
        label(title, 9f, p.argb(G7AppearanceRole.MENU_PRIMARY), true).apply {
            letterSpacing =
                .08f
            ; setPadding(4.dp, 15.dp, 4.dp, 4.dp)
        },
    )
    ...
    private fun button(
        ...
        title,
        9f,
        if (selected) p.argb(G7AppearanceRole.MENU_BACKGROUND) else p.argb(G7AppearanceRole.MENU_TEXT_PRIMARY),
        true,
    ).apply {
        gravity =
            Gravity.CENTER
        ; background =
            rounded(
                if (selected) p.argb(G7AppearanceRole.MENU_PRIMARY) else p.argb(G7AppearanceRole.MENU_SURFACE),
                p.argb(G7AppearanceRole.MENU_BORDER),
                999f,
            )
