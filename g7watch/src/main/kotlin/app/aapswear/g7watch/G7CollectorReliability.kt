...
internal object G7CollectorReliability {
    fun rehydrate(state: G7PersistedState, now: Long): G7PersistedState {
        ...
        return state.copy(health = state.health.copy(
    ...
    fun collecting(health: G7CollectorHealth) = health.copy(
        bleLink = G7BleLinkState.CONNECTING,
        runtime = G7CollectorRuntimeState.COLLECTING,
    )

    fun reachable(health: G7CollectorHealth, now: Long, connected: Boolean = false) = health.copy(
    ...
    fun authenticated(health: G7CollectorHealth, now: Long) = reachable(health, now, connected = true).copy(
        bleLink = G7BleLinkState.READY,
    )

    fun succeeded(health: G7CollectorHealth, measuredAt: Long, connectedAt: Long) = health.copy(
        sensorAvailability = G7SensorAvailability.AVAILABLE,
        bleLink = G7BleLinkState.DISCONNECTED,
    ...
    fun releasePending(health: G7CollectorHealth, now: Long) = health.copy(
    ...
    fun dataHealth(measuredAt: Long?, now: Long): G7DataHealth = when {
    ...
    private val directFailures = setOf(G7FailureClass.DIRECT_NO_CALLBACK, G7FailureClass.DIRECT_GATT_133, G7FailureClass.DIRECT_OTHER_GATT_ERROR)
...
internal fun g7FailureClass(errorCode: String, sensorAdvertisementSeen: Boolean, foreignAdvertisementsSeen: Boolean): G7FailureClass = when {
