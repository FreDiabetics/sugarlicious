...
class XdripPayloadAdapter(
    ...
    fun parse(
        ...
        val unit =
            values[XdripContract.EXTRA_UNITS]?.toString()?.lowercase().let {
...
class XdripDataSource(
    ...
    fun accept(
        values: Map<String, Any?>,
        receivedAtEpochMs: Long,
    ): TherapyDisplayState? = adapter.parse(values, receivedAtEpochMs)?.also(current::set)
