...
class AapsCapabilityDetector {
    fun detect(values: Map<String, Any?>): AapsContract =
        when {
...
class AapsDevAdapter : AapsVersionAdapter {
    override fun parse(
        values: Map<String, Any?>,
        receivedAtEpochMs: Long,
    ) = AapsPayloadAdapter.parse(values, receivedAtEpochMs)
}

class AapsStableAdapter : AapsVersionAdapter {
    override fun parse(
        values: Map<String, Any?>,
        receivedAtEpochMs: Long,
    ) = AapsPayloadAdapter.parse(values, receivedAtEpochMs)
...
class AndroidApsDataSource(
    ...
    fun accept(
        ...
        val parsed =
            when (detector.detect(values)) {
                AapsContract.DEV_EXTENDED_STATUS_V1 -> dev.parse(values, receivedAtEpochMs)
                AapsContract.STABLE_LEGACY_STATUS -> stable.parse(values, receivedAtEpochMs)
                AapsContract.UNSUPPORTED -> null
            }
        if (parsed !=
            null
        ) {
