...
class WearProtocolTest {
    ...
    @Test fun roundTrip() {
        val s = TherapyDisplayState(receivedAtEpochMs = 2, glucose = GlucoseState(100.0, GlucoseUnit.MG_DL, measuredAtEpochMs = 1))
                ...
                glucoseHistory =
                    (0 until 600).map {
                        GlucoseSample(
                            100.0 + it % 40,
                            now - (599 - it) * 300_000L,
                            sensorId = "sensor-very-long-identifier",
                            sessionId = "session-very-long-identifier",
                        )
                    },
                therapyHistory =
                    (0 until 600).map {
                        TherapyHistorySample(
                            now - (599 - it) * 300_000L,
                            totalIob = it / 100.0,
                            cobGrams = it.toDouble(),
                            basalUnitsPerHour = 0.7,
                        )
                    },
    ...
    @Test fun g7ReadingBatchRoundTripsAndDeduplicatesOnlyIdenticalIds() {
        ...
        val batch =
            G7ReadingBatch(batchId = "batch-1", readings = listOf(first, sameId, sameSequenceDifferentSession), sentAtEpochMs = 300L)
    ...
    @Test fun g7ReadingBatchRejectsWrongSourceAndOversizedPayload() {
        val wrong =
            G7ReadingBatch(
                batchId = "batch",
                readings = listOf(g7("wrong", "session", 1, 100L).copy(source = DataSourceId.ANDROID_APS)),
                sentAtEpochMs = 200L,
            )
        ...
        val oversized =
            G7ReadingBatch(
                batchId = "batch",
                readings =
                    (0..G7ReadingBatch.MAX_READINGS).map {
                        g7("id-$it", "session", it.toLong(), it.toLong())
                    },
                sentAtEpochMs = 200L,
            )
